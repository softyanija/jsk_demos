...
def callback(img_msg, rec_msg):
    ...
    if (not rec_msg.rects == []):
        ...
        if f == 1:
            ...
            under.angle = rec_msg.rects[use_rect].angle
            under.center.x = rec_msg.rects[use_rect].center.x + (memory_len / 2)* math.sin(math.radians(angle))
            under.center.y = rec_msg.rects[use_rect].center.y + (memory_len / 2)* math.cos(math.radians(angle))
            under.size.width = rec_msg.rects[use_rect].size.width
            under.size.height = rec_msg.rects[use_rect].size.height
            memory_under.rects.append(under)
...
pub_under = rospy.Publisher('/timer_cam1_rec/memory/memory_under', RotatedRectArrayStamped, queue_size=1)
