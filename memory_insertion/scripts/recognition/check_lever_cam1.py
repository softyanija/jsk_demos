...
class CheckLeverCam1():
    ...
    def callback(self, sub_socket_rect, image):
        self.sub_socket_rect = sub_socket_rect.rects
        self.sub_image = self.bridge.imgmsg_to_cv2(image, "rgb8")
        self.header = image.header


    def oneshot(self):
        if (self.sub_image is not None) and (not rect_msg.rects == []):
    ...
    def loop(self):
        rate = rospy.Rate(2)
        rospy.loginfo("start check_lever_cam1")
