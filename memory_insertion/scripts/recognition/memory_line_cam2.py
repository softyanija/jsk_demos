...
class MemoryLineCam2():
    ...
    def callback(self, lines, area, image):
    ...
    def oneshot(self):
        if (self.sub_image is not None):
            ...
            rospy.loginfo(self.sub_memory_lines)
            if ((not self.sub_memory_lines == []) and (not self.sub_memory_area == [])):
                ...
                ref_deg = math.atan(t)
                rospy.loginfo("ref_deg:{}".format(ref_deg))
                use_line = 0
                self.memory_angle = None
                ...
                for i,line in enumerate(self.sub_memory_lines):
                    ...
                    rospy.loginfo("diff_deg:{} i:{}".format(diff_deg, i))
                ...
                if (abs(diff_deg_min) < 2):
                    line = self.sub_memory_lines[use_line]
                    rospy.loginfo("start:({}, {}) end:({}, {})".format(line.pt1.x, line.pt1.y, line.pt2.x, line.pt2.y))
